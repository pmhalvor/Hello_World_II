public class Lenkeliste<T> implements Liste<T>{
    ...
    public void leseListe(){ //for my own testing purposes
        ...
        for (int i=0; i<length; i++){
            System.out.println(temp.ting + " has index: "+temp.index);
            temp = temp.neste;
        }
    }
    public void checkHodeAndHale(){ //for my own testing purposes
        System.out.println(hode.ting + " = hode");
        System.out.println(hale.ting + " = hale");
    ...
    public void leggTil(int pos, T x) throws UgyldigListeIndeks{
        ...
        if(erTom() && pos != 0){ //to avoid nullPointer exception
            ...
            for(int i = 0 ; i < stop; i++){ //Since length is increased when new Node() is created, this for-loop will go up to the correct length
                ...
                // System.out.println(curr.ting + " has index" + curr.index);
                if(nyNode == null){
    ...
    public void sett(int pos, T x) throws UgyldigListeIndeks{
        ...
        if(gyldigIndeks(pos)){
            for(int i=0; i<length; i++){
                if(temp.index == pos){
                    temp.ting = nyNode.ting; //the only thing that will change with this Node is it's ting, this makes it simpler
                }
                prev = temp;
                temp = temp.neste;
            }
